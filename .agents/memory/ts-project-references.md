...
- Do NOT trust an isolated/throwaway tsconfig that drops `references` to "verify" an artifact — without project references TS resolves lib types differently and emits misleading errors against the wrong hooks/shapes. Fix the reference graph and run the real per-package typecheck instead.
- Orval codegen contracts: GET hooks are React Query *queries*, not mutations — trigger one-shot GETs with a disabled query + `refetch()` (state via `isFetching`), and pass `queryKey: getXxxQueryKey(params)` whenever you supply a `query` options object. Mutations on a nested resource take the parent id as a SEPARATE variable (e.g. `{ clientId, data }` / `{ clientId, id }`), not folded into `data`.
